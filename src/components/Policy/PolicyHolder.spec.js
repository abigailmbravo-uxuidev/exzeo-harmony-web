...
describe('PolicyHolder', () => {
  ...
  it('renders PolicyHolder without policyID', () => {
    const wrapper = shallow(<PolicyHolderComponent activeTab="policyHolder" policyNumber="12345" policy={{}} agents={agents} />);
    expect(wrapper.exists()).toBe(true);
  });

  it('renders PolicyHolder with policy', () => {
    const wrapper = shallow(<PolicyHolderComponent activeTab="policyHolder" policyNumber="12345" policy={POLICY_TEST_DATA} agents={agents} />);
    expect(wrapper.exists()).toBe(true);
  });
});
