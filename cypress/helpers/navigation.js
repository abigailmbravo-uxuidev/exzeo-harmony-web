...
export const navigateThroughLanding = () =>
  cy.task('log', 'Navigating through Landing').get('.btn[href="/search/address"]').click();

export const navigateThroughSearchAddress = ({ address = userHO3.address } = {}) =>
  cy.task('log', 'Navigating through Searching Address')
    .get('input[name=address]').type(address)
...
export const navigateThroughPolicyholder = ({ customerInfo = userHO3.customerInfo, secondCustomerInfo = userHO3.secondCustomerInfo, agentCode = userHO3.agentCode } = {}) =>
  cy.task('log', 'Navigating through Policyholder')
    .wrap(Object.entries(customerInfo)).each(([field, value]) => cy.findDataTag(field).find('input').type(`{selectall}{backspace}${value}`))
...
export const navigateThroughUnderwriting = (data = underwritingHO3) =>
  cy.task('log', 'Navigating through Underwriting')
    .wrap(Object.entries(data)).each(([name, value]) => cy.findDataTag(`underwritingAnswers.${name}.answer_${value}`).click())
    .clickSubmit('#QuoteWorkflow').wait('@updateQuote');

export const navigateThroughCustomize = () =>
  cy.task('log', 'Navigating through Customize').clickSubmit('#QuoteWorkflow').wait('@updateQuote');
...
export const navigateThroughAssumptions = () =>
  cy.task('log', 'Navigating through Confirm Assumptions').findDataTag('confirm-assumptions').click().clickSubmit('#QuoteWorkflow');

export const navigateThroughAdditionalInterests = () =>
  cy.task('log', 'Navigating through Additional Interests').clickSubmit('#QuoteWorkflow');
...
export const navigateThroughMailingBilling = () =>
  cy.task('log', 'Navigating through Mailing Billing')
    ...
    .clickSubmit('#QuoteWorkflow').wait('@updateQuote').wait('@verifyQuote');

export const navigateThroughVerify = () =>
  cy.task('log', 'Navigating through Verify')
    .findDataTag('confirmProperty').click()
...
export const navigateThroughScheduleDate = () =>
  cy.task('log', 'Navigating through Schedule Date').clickSubmit('[data-test="schedule-date-modal"]', 'modal-submit');

export const navigateThroughThankYou = () =>
  cy.task('log', 'Navigating through Thank You')
    .get('#thanks a[href="/"]').click()
